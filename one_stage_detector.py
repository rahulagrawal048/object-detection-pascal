...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for stem_channel in stem_channels:
          ...
          conv2 = nn.Conv2d(in_channels=last_channel, out_channels=stem_channel,
                                    kernel_size=3, stride = 1, padding='same')
        ...
        # Replace "pass" statement with your code
        self.pred_cls = nn.Conv2d(in_channels=last_channel, out_channels=num_classes,
                                  kernel_size=3, stride = 1, padding='same')
        nn.init.normal_(self.pred_cls.weight, mean=0, std=0.01)
        self.pred_cls.bias.data.fill_(0)
        self.pred_box = nn.Conv2d(in_channels=last_channel, out_channels=4,
                                  kernel_size=3, stride = 1, padding='same')
        nn.init.normal_(self.pred_box.weight, mean=0, std=0.01)
        self.pred_box.bias.data.fill_(0)
        self.pred_ctr = nn.Conv2d(in_channels=last_channel, out_channels=1,
                                  kernel_size=3, stride = 1, padding='same')  
        nn.init.normal_(self.pred_ctr.weight, mean=0, std=0.01)
        self.pred_ctr.bias.data.fill_(0)                       
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key in feats_per_fpn_level:
          stem_cls_out = self.stem_cls(feats_per_fpn_level[key])
          stem_box_out = self.stem_box(feats_per_fpn_level[key])
          pred_cls_out = self.pred_cls(stem_cls_out).permute((0, 2, 3, 1))
          pred_box_out = self.pred_box(stem_box_out).permute((0, 2, 3, 1))
          pred_ctr_out = self.pred_ctr(stem_box_out).permute((0, 2, 3, 1))
          class_logits[key] = pred_cls_out.reshape(pred_cls_out.shape[0], 
                              pred_cls_out.shape[1]*pred_cls_out.shape[2],
                              pred_cls_out.shape[3])
          boxreg_deltas[key] = pred_box_out.reshape(pred_box_out.shape[0], 
                              pred_box_out.shape[1]*pred_box_out.shape[2],
                              pred_box_out.shape[3])
          centerness_logits[key] = pred_ctr_out.reshape(pred_ctr_out.shape[0], 
                              pred_ctr_out.shape[1]*pred_ctr_out.shape[2],
                              pred_ctr_out.shape[3])
...
def fcos_get_deltas_from_locations(
    ...
    # Replace "pass" statement with your code
    N = locations.shape[0]
    deltas = torch.zeros((N,4), dtype = gt_boxes.dtype, device=gt_boxes.device)
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    R = deltas[:,2]
    B = deltas[:,3]
    num = torch.minimum(L, R) * torch.minimum(T, B)
    den = torch.maximum(L, R) * torch.maximum(T, B)
...
class FCOS(nn.Module):
    ...
    def __init__(
        ...
        self.pred_net = FCOSPredictionNetwork(num_classes=self.num_classes,
                        in_channels=fpn_channels, stem_channels=stem_channels)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(
        ...
        for i in range(0,B):
          matched_box_temp = {}
          matched_box_temp = fcos_match_locations_to_gt(
        ...
        for i in range(0,B):
          matched_deltas_temp = {}
          matched_deltas_temp["p3"] = fcos_get_deltas_from_locations(locations_per_fpn_level["p3"],
                                 matched_gt_boxes[i]["p3"], self.backbone.fpn_strides["p3"])
          matched_deltas_temp["p4"] = fcos_get_deltas_from_locations(locations_per_fpn_level["p4"],
                                 matched_gt_boxes[i]["p4"], self.backbone.fpn_strides["p4"])
          matched_deltas_temp["p5"] = fcos_get_deltas_from_locations(locations_per_fpn_level["p5"],
                                 matched_gt_boxes[i]["p5"], self.backbone.fpn_strides["p5"])
          matched_gt_deltas.append(matched_deltas_temp)
        ...
        # Replace "pass" statement with your code
        matched_gt_boxes_sliced = matched_gt_boxes[:,:,4:5].flatten()
        mask = (matched_gt_boxes_sliced == -1)
        matched_gt_boxes_sliced[mask] += 1
        gt_classes = F.one_hot(matched_gt_boxes_sliced.long(), self.num_classes)
        gt_classes[mask] = 0
        gt_classes = gt_classes.reshape(pred_cls_logits.shape)
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1:
            # Replace "pass" statement with your code
            most_confident_score, most_confident_cls = torch.max(level_pred_scores, dim = 1)
            level_pred_classes = most_confident_cls[most_confident_score > test_score_thresh]
            level_pred_scores = most_confident_score[most_confident_score > test_score_thresh]

            # Step 2:
            # Replace "pass" statement with your code
            level_deltas = level_deltas[most_confident_score > test_score_thresh,:]
            level_locations = level_locations[most_confident_score > test_score_thresh,:]
            ...
            h = images.shape[2]
            w = images.shape[3]
            level_pred_boxes[:,0] = torch.clamp(level_pred_boxes[:,0], 0)
            level_pred_boxes[:,1] = torch.clamp(level_pred_boxes[:,1], 0)
            level_pred_boxes[:,2] = torch.clamp(level_pred_boxes[:,2], None, h)
            level_pred_boxes[:,3] = torch.clamp(level_pred_boxes[:,3], None, w)
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
