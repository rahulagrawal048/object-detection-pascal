...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for stem_channel in stem_channels:
          ...
          conv = nn.Conv2d(in_channels=self.last_channel, out_channels=stem_channel,
                                    kernel_size=3, stride = 1, padding='same')
          nn.init.normal_(conv.weight, mean=0, std=0.01)
          conv.bias.data.fill_(0)
          stem_rpn.append(conv)
          stem_rpn.append(nn.ReLU())
          self.last_channel = stem_channel
        ...
        # Replace "pass" statement with your code
        self.pred_obj =  nn.Conv2d(in_channels=self.last_channel, out_channels=self.num_anchors,
                                  kernel_size=1, stride = 1, padding='same')
        nn.init.normal_(self.pred_obj.weight, mean=0, std=0.01)
        self.pred_obj.bias.data.fill_(0)
        self.pred_box =  nn.Conv2d(in_channels=self.last_channel, out_channels=4*self.num_anchors,
                                  kernel_size=1,stride = 1, padding='same')    
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key in feats_per_fpn_level:
          stem_rpn_out = self.stem_rpn(feats_per_fpn_level[key])
          pred_obj_out = self.pred_obj(stem_rpn_out).permute((0, 2, 3, 1))
          pred_box_out = self.pred_box(stem_rpn_out).permute((0, 2, 3, 1))
          object_logits[key] = pred_obj_out.reshape(pred_obj_out.shape[0], 
                              pred_obj_out.shape[1]*pred_obj_out.shape[2]*self.num_anchors)
          boxreg_deltas[key] = pred_box_out.reshape(pred_box_out.shape[0], 
                              pred_box_out.shape[1]*pred_box_out.shape[2]*self.num_anchors,
                              int(pred_box_out.shape[3]/self.num_anchors))
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def generate_fpn_anchors(
    ...
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            anchor_box = torch.zeros((locations.shape[0], 4), dtype = locations.dtype,
                                      device = locations.device)
            anchor_box[:,0] =  locations[:,0] - new_width / 2
            anchor_box[:,1] =  locations[:,1] - new_height / 2
            anchor_box[:,2] =  locations[:,0] + new_width / 2
            anchor_box[:,3] =  locations[:,1] + new_height / 2
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    ix1 = boxes1[:,0].repeat(N,1).t()
    iy1 = boxes1[:,1].repeat(N,1).t()
    ix2 = boxes1[:,2].repeat(N,1).t()
    iy2 = boxes1[:,3].repeat(N,1).t()
    xx1 = torch.max(ix1, boxes2[:, 0].repeat(M,1))
    yy1 = torch.max(iy1, boxes2[:, 1].repeat(M,1))
    xx2 = torch.min(ix2, boxes2[:, 2].repeat(M,1))
    yy2 = torch.min(iy2, boxes2[:, 3].repeat(M,1))
    w = torch.max(torch.zeros_like(xx2), xx2 - xx1)
    h = torch.max(torch.zeros_like(yy2), yy2 - yy1)
    inter = w * h
    iou = inter/(area_boxes1.repeat(N,1).t() + area_boxes2.repeat(M,1) - inter)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return iou
...
def rcnn_get_deltas_from_anchors(
    ...
    bx = gt_boxes[:, 0] + (gt_boxes[:,2] - gt_boxes[:,0])/2
    by = gt_boxes[:, 1] + (gt_boxes[:,3] - gt_boxes[:,1])/2
    bw = gt_boxes[:,2] - gt_boxes[:,0]
    bh = gt_boxes[:,3] - gt_boxes[:,1]

    px = anchors[:, 0] + (anchors[:,2] - anchors[:,0])/2
    py = anchors[:, 1] + (anchors[:,3] - anchors[:,1])/2
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        anchors_per_fpn_level = generate_fpn_anchors(locations_per_fpn_level, strides_per_fpn_level, self.anchor_stride_scale,
        ...
        for i in range(0, B):
          matched_box_temp = {}
          matched_box_temp = rcnn_match_anchors_to_gt(
                anchor_boxes, gt_boxes[i,:], self.anchor_iou_thresholds)
          matched_gt_boxes.append(matched_box_temp)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        if self.training:
            ...
            gt_objectness = torch.zeros(pred_obj_logits.shape, device = fg_idx.device)
    ...
    def predict_proposals(
        ...
        for _batch_idx in range(batch_size):
            ...
            for level_name in anchors_per_fpn_level.keys():
                ...
                proposal_boxes = rcnn_apply_deltas_to_anchors(level_boxreg_deltas,
                                                                level_anchors)
                proposal_boxes[:,0] = torch.clamp(proposal_boxes[:,0], 0, w)
                proposal_boxes[:,1] = torch.clamp(proposal_boxes[:,1], 0, h)
                proposal_boxes[:,2] = torch.clamp(proposal_boxes[:,2], 0, w)
                proposal_boxes[:,3] = torch.clamp(proposal_boxes[:,3], 0, h)

                #sorted_obj, indices = torch.sort(level_obj_logits, descending=True, stable = True)
                #proposal_boxes = proposal_boxes[indices, :] 

                if(level_obj_logits.shape[0] < self.pre_nms_topk):
                  self.pre_nms_topk = level_obj_logits.shape[0]

                #proposal_boxes = torch.topk(proposal_boxes, self.pre_nms_topk, 0)[0]
                #sorted_obj = torch.topk(sorted_obj, self.pre_nms_topk, 0)[0]
                sorted_obj, indices = torch.topk(level_obj_logits, self.pre_nms_topk)
                proposal_boxes = proposal_boxes[indices, :] 

                keep = torchvision.ops.nms(proposal_boxes, sorted_obj, self.nms_thresh)
                proposals = proposal_boxes[keep,:]
                scores = sorted_obj[keep]
                proposals_per_fpn_level_per_image[level_name] = proposals
                scores_per_fpn_level_per_image[level_name] = scores

                #if(proposals.shape[0] < self.post_nms_topk):
                #  self.post_nms_topk = proposals.shape[0]
                #proposals = torch.topk(proposals, self.post_nms_topk, 0)[0]
                ##############################################################
                #                        END OF YOUR CODE                    #
                ##############################################################
...
class FasterRCNN(nn.Module):
    ...
    def __init__(
        ...
        cls_pred = []
        # Replace "pass" statement with your code
        last_channel = self.backbone.out_channels
        for stem_channel in stem_channels:
          #Initialize conv layer
          conv = nn.Conv2d(in_channels=last_channel, out_channels=stem_channel,
                                    kernel_size=3, stride = 1, padding='same')
          nn.init.normal_(conv.weight, mean=0, std=0.01)
          conv.bias.data.fill_(0)
          cls_pred.append(conv)
          cls_pred.append(nn.ReLU())
          last_channel = stem_channel
        ...
        lin = nn.Linear(last_channel*self.roi_size[0]*self.roi_size[0],
                                   self.num_classes + 1)
        nn.init.normal_(lin.weight, mean=0, std=0.01)
    ...
    def forward(
        ...
        for level_name in feats_per_fpn_level.keys():
            ...
            # Replace "pass" statement with your code
            roi_feats = torchvision.ops.roi_align(level_feats, level_props, self.roi_size,
                                        aligned=True, spatial_scale=1/level_stride)
            ##################################################################
            #                         END OF YOUR CODE                       #
            ##################################################################
        ...
        for _idx in range(len(gt_boxes)):
            ...
            # Replace "pass" statement with your code
            matched_gt_boxes.append(rcnn_match_anchors_to_gt(proposals_per_image, gt_boxes_per_image,
                                                      (0.6, 0.6)))
        ...
        idx = torch.cat((fg_idx, bg_idx))
        class_labels = matched_gt_boxes[:,4].type(torch.LongTensor)
       
        class_labels = class_labels[idx]
        class_labels += 1
        pred_cls_logits = pred_cls_logits[idx,:]
        loss_cls = F.cross_entropy(pred_cls_logits, class_labels.to(pred_cls_logits.device))
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        return {
    ...
    def inference(
        ...
        softmax_scores = F.softmax(pred_cls_logits, dim = 1)
        softmax_scores = softmax_scores[:,1:]

        most_confident_score, most_confident_cls = torch.max(softmax_scores, dim = 1)

        pred_classes = most_confident_cls[most_confident_score > test_score_thresh]
        pred_scores = most_confident_score[most_confident_score > test_score_thresh]
        pred_boxes = pred_boxes[most_confident_score > test_score_thresh]
